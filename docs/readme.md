...
- OTA 使用两个4KB VP缓冲区：A=`0x7000..0x77FF`，B=`0x6800..0x6FFF`。
  升级期间这两个区域必须专用，页面变量或其他逻辑不能改写。B避开了旧实机记录中出错的 `0x7800` 区域，仍需在目标屏工程上验证。
- 每包写入VP并回读校验后，异步启动NAND写入，立即请求下一包；下一包在另一VP区暂存后，才等待上一包写入并检查块CRC。
  保留擦除边界的一次重试，重试读取上一块VP，不会使用下一包数据。最后一包必须完成写入、块CRC和文件CRC后才返回成功；进度按已校验写入的字节更新。
